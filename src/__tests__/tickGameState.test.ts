...
// ── mutating interface (tracer bullet) ───────────────────────────────────────

describe('tickGameState — mutating interface', () => {
  it('returns GameEvent[] and mutates ship velocity in place', () => {
    const state = { ...emptyState(), ship: { ...baseShip()!, rotation: 0 } }
    const result = tickGameState(state, { ...noInput, thrust: true }, 1)
    expect(Array.isArray(result)).toBe(true)
    expect(state.ship!.vel.x).toBeCloseTo(SHIP_THRUST)
  })
})
...
describe('tickGameState stub', () => {
  it('returns empty events for empty state', () => {
    const state = emptyState()
    const events = tickGameState(state, noInput, 1)
...
describe('tickGameState — ship physics', () => {
  it('thrust increases velocity in the direction of rotation', () => {
    const state = { ...emptyState(), ship: { ...baseShip()!, rotation: 0 } }
    tickGameState(state, { ...noInput, thrust: true }, 1)
    expect(state.ship!.vel.x).toBeCloseTo(SHIP_THRUST)
    expect(state.ship!.vel.y).toBeCloseTo(0, 5)
  })

  it('left input rotates ship counter-clockwise', () => {
    const state = { ...emptyState(), ship: { ...baseShip()!, rotation: 0 } }
    tickGameState(state, { ...noInput, left: true }, 1)
    expect(state.ship!.rotation).toBeCloseTo(-SHIP_ROTATION_SPEED)
  })

  it('right input rotates ship clockwise', () => {
    const state = { ...emptyState(), ship: { ...baseShip()!, rotation: 0 } }
    tickGameState(state, { ...noInput, right: true }, 1)
    expect(state.ship!.rotation).toBeCloseTo(SHIP_ROTATION_SPEED)
  })

  it('drag is applied each frame', () => {
    const state = { ...emptyState(), ship: { ...baseShip()!, vel: { x: 1, y: 0 } } }
    tickGameState(state, noInput, 1)
    expect(state.ship!.vel.x).toBeCloseTo(Math.pow(SHIP_DRAG, 1))
  })
...
describe('tickGameState — asteroid movement', () => {
  it('position advances by vel * speedMult', () => {
    ...
    expect(state.asteroids[0].pos.x).toBeCloseTo(101)
  })
  ...
  it('wraps x > CANVAS_W + radius to left off-screen edge', () => {
    ...
    tickGameState(state, noInput, 1)
    expect(state.asteroids[0].pos.x).toBeLessThan(0)
  })
})
...
describe('tickGameState — bullet lifetime', () => {
  it('active bullet lifetime decrements by dt', () => {
    ...
    tickGameState(state, noInput, 1)
    expect(state.bullets[0].lifetime).toBeCloseTo(BULLET_LIFETIME - 1)
  })
  ...
  it('inactive bullet slot is not updated', () => {
    ...
    tickGameState(state, noInput, 1)
    expect(state.bullets[0].lifetime).toBe(BULLET_LIFETIME)
  })
})
...
describe('tickGameState — UFO movement', () => {
  ...
  it('UFO position advances each frame', () => {
    ...
    tickGameState(state, noInput, 1)
    expect(state.ufo!.pos.x).toBeCloseTo(201.5)
  })

  it('UFO fire timer decrements each frame', () => {
    const state = { ...emptyState(), ufo: baseUfo(), ship: baseShip() }
    tickGameState(state, noInput, 1)
    expect(state.ufo!.fireTimer).toBeCloseTo(UFO_FIRE_RATE - 1)
  })
  ...
  it('UFO adds an active UFO bullet slot when fire timer expires', () => {
    ...
    tickGameState(state, noInput, 1)
    expect(state.ufoBullets.some(b => b.active)).toBe(true)
  })
...
describe('tickGameState — collision response', () => {
  ...
  it('asteroid hitting ship with Shield absorbs hit — no loseLife, Shield cleared', () => {
    ...
    expect(state.ship!.powerUp).toBeNull()
  })
})
...
describe('tickGameState — power-up timer', () => {
  it('SpreadShot timer expiring clears powerUp on ship in state', () => {
    const state = {
      ...emptyState(),
...
describe('tickGameState — bullet firing', () => {
  it('fire with cooldown ready activates one bullet slot', () => {
    ...
    tickGameState(state, fireInput, 1)
    expect(state.bullets.filter(b => b.active)).toHaveLength(1)
  })
  ...
  it('fired bullet starts at ship position', () => {
    ...
    tickGameState(state, fireInput, 1)
    const fired = state.bullets.find(b => b.active)!
    expect(fired.pos.x).toBeCloseTo(300)
    expect(fired.pos.y).toBeCloseTo(200)
  })

  it('sets fireCooldown > 0 on ship in state after firing', () => {
    const state = { ...emptyState(), ship: baseShip(), bullets: inactiveBullets(5) }
    tickGameState(state, fireInput, 1)
  ...
  it('SpreadShot activates 3 bullet slots', () => {
    ...
    tickGameState(state, fireInput, 1)
    expect(state.bullets.filter(b => b.active)).toHaveLength(3)
  })
})
